...
export default function EventDetailPage() {
    ...
    return (
      <div className="bg-background flex items-center justify-center p-4 py-20">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-6">This event may have passed or been removed.</p>
  ...
  const getMapsUrl = () => {
    const query = event.venue.address 
      ? `${event.venue.name}, ${event.venue.address}, Chicago, IL`
      ...
      <div className="relative w-full">
        <div className="relative w-full aspect-square sm:aspect-[16/9] lg:aspect-[21/9]">
          {!imageError ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-primary via-secondary to-primary flex items-center justify-center">
          ...
          <div className="absolute top-4 left-4 flex gap-2">
            ...
            {isTomorrow && (
              <Badge className="bg-secondary text-white font-bold shadow-lg text-sm px-3 py-1">Tomorrow</Badge>
            )}
            {event.isFestival && (
      ...
      <div className="px-4 sm:px-6 py-8 max-w-3xl lg:max-w-4xl mx-auto">
        <div className="space-y-8">
          ...
          <div className="space-y-8">
            ...
            {/* Header Section */}
            <div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-foreground mb-3 leading-tight">
                {event.headliner}
              </h1>
            ...
            {event.artists.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-bold text-foreground">Full Lineup</h2>
                    ...
                    return (
                      <Badge 
                        ...
                        className={isHeadliner 
                          ...
                          : "bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-500 border border-cyan-500/20 px-4 py-2 text-sm font-medium"
                        }
                      >
                        {artist.name}
                        {artist.b2b && " (B2B)"}
                ...
                {event.ages && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{event.ages}</p>
                    </div>
                  </div>
                )}
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{event.artists.length} Artist{event.artists.length > 1 ? "s" : ""}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
              ...
              <a 
                ...
                className="flex items-center justify-center gap-3 w-full py-4 px-6 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-lg transition-colors"
              >
              ...
              <div className="rounded-2xl bg-card border border-border overflow-hidden">
                ...
                <div className="p-5 space-y-4">
                  <Link href={`/venue/${venueSlug}`} className="block group">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">{event.venue.location}</p>
                    {event.venue.address && (
                  ...
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-3 px-4 rounded-xl border border-border hover:bg-muted transition-colors text-sm font-medium"
                  >
              ...
              <div className="p-5 rounded-2xl bg-card border border-border">
                <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-4">Share Event</h3>
                <div className="flex gap-2">
                  <button
                    onClick={handleShare}
                    className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-muted hover:bg-muted/80 transition-colors text-sm font-medium"
                  >
                    <Share2 className="w-4 h-4" />
                    Share
                  </button>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center p-3 rounded-xl bg-muted hover:bg-muted/80 transition-colors"
                  >
              ...
              <Link 
                ...
                className="flex items-center justify-between p-4 rounded-xl border border-border hover:border-primary/50 hover:bg-card transition-all group"
              >
                <span className="text-sm font-medium">More events at {event.venue.name}</span>
